...
College football transfer portal analysis tool. Uses historical transfer data (2021–2025) to score, rank, and predict which programs best develop players — surfaced through an interactive dashboard.
...
python run.py --aggregate  # portal scores + calibration table → data/processed/
python run.py --predict    # FBS portal class rankings         → data/processed/
...
unused/
    aggregate.py                  # portal scores + calibration table
    predict.py                    # FBS portal class rankings
```
...
fetch.py   →  data/raw/transfers.csv, player_stats.csv, recruits.csv,
               ppa.csv, usage.csv, sp_ratings.csv, games_played.csv
...
aggregate.py →  data/processed/portal_scores.csv
                data/processed/calibration.csv

predict.py  →  data/processed/predictions_2025.csv
```
...
**Success factor** = reliability weight (0–1) on each transfer row. Tier 1 players (met volume thresholds) weighted by games played + pre-score magnitude. Tier 2 (possible injury year) and Tier 3 (never played) get lower base weights, boosted slightly by recruiting stars.
...
### Coming soon: second algorithm for comparison
A second ranking algorithm is in development and will run alongside PageRank to produce comparable, side-by-side results. The goal is to show where the two methods agree (high confidence) and where they diverge (worth investigating).
